...
export default function App() {
  ...
  const ctrl = useDialKitController("Binary halftone", {
    ...
    persist: { key: "binary-halftone", storage: "localStorage", presets: true },
    onAction,
    shortcuts: {
  ...
  const allPresets = { ...PS.BUILT_IN, ...saved };

  function applyPreset(name) {
    const preset = allPresets[name];
    if (!preset) return;
    ctrlRef.current.setValues(preset);
    setSelected(name);
    const framed = PS.FRAME_KEYS.some(k => k in preset);
    setStatus({ msg: 'Applied "' + name + '"' + (framed ? " (incl. framing)" : " (style only)"), err: false });
  }
  function handleSave() {
    const name = presetName.trim();
    if (!name) { setStatus({ msg: "Give the preset a name first.", err: true }); return; }
    if (name in PS.BUILT_IN) { setStatus({ msg: "That name is a built-in — pick another.", err: true }); return; }
    setSaved(PS.savePreset(name, ctrlRef.current.getValues(), includeFraming));
    setSelected(name);
    setPresetName("");
    setStatus({ msg: 'Saved "' + name + '" to this browser.', err: false });
  }
  function handleDelete() {
    if (!selected || selected in PS.BUILT_IN) {
      setStatus({ msg: "Select one of your saved presets to delete.", err: true });
      return;
    }
    setSaved(PS.deletePreset(selected));
    setSelected("");
    setStatus({ msg: "Preset deleted.", err: false });
  }
  function handleExportPresets() {
    const blob = new Blob([PS.exportJSON(saved)], { type: "application/json" });
    const a = document.createElement("a");
    a.href = URL.createObjectURL(blob);
    a.download = "halftone-presets.json";
    a.click();
    setTimeout(() => URL.revokeObjectURL(a.href), 2000);
    setStatus({ msg: "Exported " + Object.keys(saved).length + " saved preset(s).", err: false });
  }
  function handleImport(file) {
    ...
    r.onload = () => {
      try {
        setSaved(PS.importJSON(String(r.result)));
      ...
      <aside className="panel">
        <header className="phead">
          ...
          <div className="presets">
            <div className="prow">
              <select value={selected} onChange={e => applyPreset(e.target.value)}>
                <option value="">Preset...</option>
                <optgroup label="Built-in (shared)">
                  {Object.keys(PS.BUILT_IN).map(n => <option key={n} value={n}>{n}</option>)}
                </optgroup>
            ...
            <label className="pcheck">
              <input type="checkbox" checked={includeFraming}
                     onChange={e => setIncludeFraming(e.target.checked)} />
              Include framing (source, placement, inflate)
            </label>
            <div className="prow3">
              <button onClick={handleShare}>Share link</button>
              <button onClick={handleExportPresets}>Export</button>
            ...
            <div className="prow3">
              ...
              <button onClick={handleReset}>Baseline</button>
              <button onClick={() => ctrlRef.current.resetValues()}>Defaults</button>
            </div>
          </div>
        </header>
